...
class Decoder(nn.Module):
    def __init__(self, num_particles, encoder_hidden_size, decoder_hidden_size, T, fc_encoder_final, feats=1, pf=False):
    ...
    def resampling(self, num_particles, particles, prob):
        """
        The implementation of soft-resampling. We implement soft-resampling in a batch-manner.
    ...
    def forward(self, input_encoded, y_prev):
        if self.pf:
            ...
            for t in range(self.T):
                ...
                prob = torch.exp(prob)
                prob = prob/torch.sum(prob, dim=1, keepdim=True)
                hiddens, cells = self.resampling(self.num_particles, (hiddens.view(-1, self.decoder_hidden_size), cells.view(-1, self.decoder_hidden_size)), prob)
...
class DA_RNN(nn.Module):
    def __init__(self, X, y, T, num_particles, encoder_hidden_size, decoder_hidden_size, batch, lr, epochs, encoder_pf, decoder_pf, parallel):
        ...
        self.Decoder = Decoder(num_particles=num_particles, encoder_hidden_size=encoder_hidden_size, decoder_hidden_size=decoder_hidden_size, T=T, fc_encoder_final=self.Encoder.fc_encoder_final, pf=self.decoder_pf).to(self.device)
